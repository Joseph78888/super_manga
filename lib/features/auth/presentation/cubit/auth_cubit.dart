...
class AuthCubit extends Cubit<AuthState> {
  ...
  Future<bool> authenticate({
    ...
    developer.log(
      'Starting authentication process. Mode: ${state.mode.name}',
      name: 'auth_cubit',
    );
    ...
    try {
      ...
      if (state.isLoginMode) {
        // Sign In
        developer.log(
          'Attempting sign in for user: $email',
          name: 'auth_cubit',
        );
        ...
        developer.log(
          'Sign in successful. User ID: ${user.id}',
          name: 'auth_cubit',
        );
      } else {
        // Sign Up
        developer.log(
          'Attempting sign up for user: $email',
          name: 'auth_cubit',
        );
        if (username == null || username.isEmpty) {
          developer.log(
            'Validation failed: Username required for signup',
            name: 'auth_cubit',
            level: 800,
          );
          emit(
        ...
        developer.log(
          'Sign up successful. User ID: ${user.id}',
          name: 'auth_cubit',
        );
      }
  ...
  Future<void> signInWithGoogle() async {
    ...
    try {
      ...
      developer.log(
        'Google OAuth flow started successfully',
        name: 'auth_cubit',
      );
      emit(state.copyWith(isLoading: false));
    } on AuthException catch (e, s) {
